    ...
    while (*s) {
      ...
      *endptr = s++;
      ...
      case 't':
        ...
        o = JsonValue(GASON_TRUE);
        s += 3;
        break;
      case 'f':
        if (!(s[0] == 'a' && s[1] == 'l' && s[2] == 's' && s[3] == 'e' && isdelim(s[4])))
          return JSON_PARSE_BAD_IDENTIFIER;
        o = JsonValue(GASON_FALSE);
        s += 4;
        break;
      case 'n':
      ...
      case ']':
        ...
        if (tags[pos] != GASON_ARRAY)
          return JSON_PARSE_MISMATCH_BRACKET;
        o = listToValue(GASON_ARRAY, tails[pos--]);
        break;
      case '}':
        if (pos == -1)
          return JSON_PARSE_STACK_UNDERFLOW;
        if (tags[pos] != GASON_OBJECT)
          return JSON_PARSE_MISMATCH_BRACKET;
        if (keys[pos] != nullptr)
          return JSON_PARSE_UNEXPECTED_CHARACTER;
        o = listToValue(GASON_OBJECT, tails[pos--]);
        break;
      case '[':
        if (++pos == JSON_STACK_SIZE)
          return JSON_PARSE_STACK_OVERFLOW;
        tails[pos] = nullptr;
        tags[pos] = GASON_ARRAY;
        keys[pos] = nullptr;
        separator = true;
        continue;
      case '{':
