...
/** tag (type) of each JSon element. */
  enum JsonTag {
    GASON_NUMBER = 0,        ///< double (floating point) value
    GASON_STRING,            ///< string value
    GASON_ARRAY,             ///< an array value
    GASON_OBJECT,            ///< an object value
    GASON_TRUE,              ///< true value
    GASON_FALSE,             ///< false value
    GASON_NULL = 0xF         ///< null or invalid value
  };
  ///////////////////////////////////////////////////////////////////////////////
  struct JsonNode;
  ///////////////////////////////////////////////////////////////////////////////
  ...
  struct JsonValue {
    ...
    JsonValue(JsonTag tag = GASON_NULL, void *payload = nullptr) {
    ...
    bool        isNumber() const {
      return getTag() == GASON_NUMBER;
    }
    bool        isBoolean() const {
      return getTag() == GASON_TRUE || getTag() == GASON_FALSE;
    }
    bool        isString() const {
      return getTag() == GASON_STRING;
    }
    bool        isNode() const {
      return getTag() == GASON_ARRAY || getTag() == GASON_OBJECT;
    }
    bool        isArray() const {
      return getTag() == GASON_ARRAY;
    }
    bool        isObject() const {
      return getTag() == GASON_OBJECT;
    }
    bool        isDouble() const {
      return (int64_t)ival <= (int64_t)JSON_VALUE_NAN_MASK;
    }
    JsonTag     getTag() const {
      return isDouble() ? GASON_NUMBER : JsonTag((ival >> JSON_VALUE_TAG_SHIFT) & JSON_VALUE_TAG_MASK);
    }
